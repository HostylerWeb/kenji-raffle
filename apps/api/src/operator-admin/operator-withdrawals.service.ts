...
export class OperatorWithdrawalsService {
  ...
  async listWithdrawals(operatorId: string, status?: string) {
    const client = await this.tenantConnection.getClient(operatorId);
    const rows = await client.withdrawals.findMany({
      where: status
    ...
    return rows.map((w) => ({
      ...
      source: w.claim?.winner
        ...
        : w.claim?.award
          ? `Instant win — ticket ${w.claim.award.ticket.ticket_number}`
          : "Site credit",
    }));
  }
